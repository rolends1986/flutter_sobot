...
- (void)creatButtonForArray:(NSArray *)titles tags:(NSArray *) tagValues{
    ...
    CGFloat mx = 25.0f*[self getSourceViewWidth]/375;
    CGFloat my = 15.0f;
...
-(IBAction)buttonClick:(UIButton *)sender{
    if(sender.tag==BUTTON_CONNECT_USER){
        ...
        if (![self getZCLibConfig].isArtificial) {
            ...
            if ([self getZCLibConfig].type == 2 && [self getZCkitinfo].isOpenRobotVoice && curKeyBoardStatus == WAITSTATUS) {
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    [ZCSobotCore imagePickerController:_zc_imagepicker didFinishPickingMediaWithInfo:info WithView:_zc_sourceView Delegate:_delegate block:^(NSString *filePath, ZCMessageType type, NSDictionary *duration) {
        ...
        [_skillSetView setItemClickBlock:^(ZCLibSkillSet *itemModel) {
            ...
            // 点击技能组弹框上的留言跳转
                ...
                if(keyboard.pagedelegate && [keyboard.pagedelegate respondsToSelector:@selector(keyboardItemClick:object:)]){
                    [keyboard.pagedelegate keyboardItemClick:ZCKeyboardOnClickTurnUser object:@"clearskillId"];
                }
            }
        }];
