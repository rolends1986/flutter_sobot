...
//REGULAREXPRESSION(URLRegularExpression,@"((http[s]{0,1}|ftp)://[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z0-9]{1,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)|(www.[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)")
REGULAREXPRESSION(URLRegularExpression,@"((http[s]{0,1}|ftp)://[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z0-9]{1,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)|(([a-zA-Z0-9]{2,4}).[a-zA-Z0-9\\.\\-]+\\.([a-zA-Z]{2,4})(:\\d+)?(/[a-zA-Z0-9\\.\\-~!@#$%^&*+?:_/=<>]*)?)")

REGULAREXPRESSION(PhoneNumerRegularExpression, @"\\d{3}-\\d{8}|\\d{3}-\\d{7}|\\d{4}-\\d{8}|\\d{4}-\\d{7}|1+[3578]+\\d{9}|400\\d{7}|\\d{8}|\\d{7}")
...
const CGFloat zckLineSpacing = 4.0;
const CGFloat zckAscentDescentScale = 0.25; //在这里的话无意义，高度的结局都是和宽度一样

const CGFloat zckEmojiWidthRatioWithLineHeight = 1.15;//和字体高度的比例

const CGFloat zckEmojiOriginYOffsetRatioWithLineHeight = 0.10; //表情绘制的y坐标矫正值，和字体高度的比例，越大越往下
NSString *const zckCustomGlyphAttributeImageName = @"CustomGlyphAttributeImageName";

#define kEmojiReplaceCharacter @"\uFFFC"

#define kURLActionCount 5
NSString * const zckURLActions[] = {@"url->",@"email->",@"phoneNumber->",@"at->",@"poundSign->"};

/**
 *  搞个管理器，否则自定义plist的话，每个label都会有个副本很操蛋
 */
@interface ZCMLEmojiLabelRegexPlistManager : NSObject
...
@implementation ZCMLEmojiLabelRegexPlistManager

+ (instancetype)sharedInstance {
    static ZCMLEmojiLabelRegexPlistManager *_sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
@interface ZCTTTAttributedLabel(ZCMLEmojiLabel)

@property (readwrite, nonatomic, strong) ZCTTTAttributedLabelLink *activeLink;
...
@interface ZCMLEmojiLabel()

@property (nonatomic, weak) NSRegularExpression *customEmojiRegularExpression;
@property (nonatomic, weak) NSDictionary *customEmojiDictionary; //这玩意如果有也是在ZCMLEmojiLabelPlistManager单例里面存着
...
+ (NSDictionary *)emojiDictionary {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
//        NSArray *arr = [ZCUITools allExpressionArray];
//        if(arr && arr.count>0){
//            NSMutableDictionary *dict = [NSMutableDictionary dictionary];
//            for (NSDictionary *item in arr) {
//                [dict setObject:item[@"VALUE"] forKey:item[@"KEY"]];
//            }
            emojiDictionary = [[ZCUIConfigManager getInstance] allExpressionDict];
//        }
//        NSString *emojiFilePath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"ZCEmojiExpression.bundle/ZCEmojiExpression.plist"];
...
- (void)commonInit {
    ...
    self.lineSpacing = zckLineSpacing; //默认行间距
    ...
    //点击时候的背景色
    [mutableActiveLinkAttributes setValue:(__bridge id)[[UIColor colorWithWhite:0.631 alpha:1.000] CGColor] forKey:(NSString *)kZCTTTBackgroundFillColorAttributeName];
...
-(void)setLinkColor:(UIColor *)color{
    ...
    //点击时候的背景色
    [mutableActiveLinkAttributes setValue:(__bridge id)[[UIColor clearColor] CGColor] forKey:(NSString *)kZCTTTBackgroundFillColorAttributeName];
           ...
           context:(CGContextRef)c
{
    ...
    CGFloat emojiWith = self.font.lineHeight*zckEmojiWidthRatioWithLineHeight;
    CGFloat emojiOriginYOffset = self.font.lineHeight*zckEmojiOriginYOffsetRatioWithLineHeight;
    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns(line)) {
            ...
            //判断是否有图像，如果有就绘制上去
            NSString *imageName = attributes[zckCustomGlyphAttributeImageName];
            if (imageName) {
                ...
                // 原代码 ZCEmojiExpression.bundle 通过这个获取，2.5.6版本修改文件路径
//                NSString *imagePath = [self.customEmojiBundleName?:@"ZCEmojiExpression.bundle" stringByAppendingPathComponent:imageName];
//                UIImage *image = [UIImage imageNamed:imagePath];
                UIImage * image = [ZCUITools zcuiGetExpressionBundleImage:[NSString stringWithFormat:@"%@.png",imageName]];
...
- (NSMutableAttributedString*)mutableAttributeStringWithEmojiText:(NSAttributedString *)emojiText
{
    ...
    if (self.customEmojiRegularExpression) {
        ...
        emojis = [kEmojiRegularExpression() matchesInString:emojiText.string
                                                         options:NSMatchingWithTransparentBounds
                                                           range:NSMakeRange(0, [emojiText length])];
...
//                                                           range:NSMakeRange(0, [emojiText length])];
    ...
    for (NSTextCheckingResult *result in emojis) {
        ...
        NSDictionary *emojiDict = self.customEmojiRegularExpression?self.customEmojiDictionary:[ZCMLEmojiLabel emojiDictionary];
        ...
        if (imageName) {
            ...
            metrics->ascent = 1/(1+zckAscentDescentScale)*metrics->width;
            metrics->descent = metrics->ascent*zckAscentDescentScale;
            ...
            // 设置自定义属性，绘制的时候需要用到
            [attrStr addAttribute:zckCustomGlyphAttributeImageName
                            value:imageName
                            range:__range];
        } else {
...
- (void)setText:(id)text
{
    ...
    for (NSUInteger i=0; i<maxIndex; i++) {
        ...
        NSString *urlAction = zckURLActions[i];
...
-(NSMutableDictionary *) getTextADict:(NSString *) searchText{
    ...
    NSString *tempText = searchText;
    NSMutableDictionary *resultDict = [NSMutableDictionary dictionary];
    NSMutableArray *resultArray = [NSMutableArray array];
    NSString *regex_text = @"<a.*?</a>";
    ...
    for (NSTextCheckingResult *match in arrayOfAllMatches)
    {
        ...
        NSString *replaceText = substringForMatch;
        if (!_isGuideCell) {
           replaceText = [substringForMatch stringByReplacingOccurrencesOfString:@" " withString:@""];
        }
       
        NSArray *tempArr = [replaceText componentsSeparatedByString:@"href="];
        
        NSString *href = @"";
        NSString *content = @"";
        if(tempArr && tempArr.count>1){
            NSString *temphref = [tempArr objectAtIndex:tempArr.count-1];
//            NSString *temphref = [tempArr objectAtIndex:1];
            if (!_isGuideCell) {
                if([temphref hasPrefix:@"'"]||[temphref hasPrefix:@" "]||[temphref hasPrefix:@"\""]){
                    temphref = [temphref substringFromIndex:1];
                }
            }else{
                if([temphref hasPrefix:@"'"]||[temphref hasPrefix:@"\""]){
                    temphref = [temphref substringFromIndex:1];
                }
            }
            ...
            int toIndex = 0;
            NSRange range;
            range = [temphref rangeOfString:@"'"];
            if (range.location != NSNotFound) {
                toIndex = (int)range.location;
            }
            if(toIndex==0){
                range = [temphref rangeOfString:@"\""];
                if (range.location != NSNotFound) {
            ...
            if(toIndex>0){
                href = [temphref substringToIndex:toIndex];
            }
            
            int fromIndex=0;
            range = [temphref rangeOfString:@">"];
            if (range.location != NSNotFound) {
                fromIndex = (int)range.location+1;
            }
            range = [temphref rangeOfString:@"<"];
            if (range.location != NSNotFound) {
                toIndex = (int)range.location;
            ...
            if(fromIndex>1 && toIndex>fromIndex){
                content = [temphref substringWithRange:NSMakeRange(fromIndex, toIndex-fromIndex)];
            }
            
            
            
            NSMutableDictionary *dict = [NSMutableDictionary dictionary];
//            range = [replaceText rangeOfString:href];
            //            NSLog(@"%@\n%@",NSStringFromRange(range),[replaceText substringWithRange:range]);
            
            [dict setObject:href forKey:@"url"];
            
//            range = [replaceText rangeOfString:content];
            //            NSLog(@"%@\n%@",NSStringFromRange(range),[replaceText substringWithRange:range]);
            
            
            [dict setObject:content forKey:@"htmlText"];
            [dict setObject:[NSString stringWithFormat:@"%zd",match.range.location - len] forKey:@"realFromIndex"];
            [resultArray addObject:dict];
            
            tempText = [tempText stringByReplacingCharactersInRange:NSMakeRange(match.range.location-len, match.range.length) withString:content];
            
            int x = (int)(substringForMatch.length - content.length);
            len = len + x;
        }
    ...
    if(resultArray.count>0){
        [resultDict setObject:tempText forKey:@"text"];
        [resultDict setObject:resultArray forKey:@"arr"];
        return  resultDict;
    }
    return nil;
}
...
- (void)setCustomEmojiRegex:(NSString *)customEmojiRegex
{
    ...
    if (customEmojiRegex&&customEmojiRegex.length>0) {
        self.customEmojiRegularExpression = [[ZCMLEmojiLabelRegexPlistManager sharedInstance]regularExpressionForRegex:customEmojiRegex];
    }else{
...
- (void)setCustomEmojiPlistName:(NSString *)customEmojiPlistName
{
    ...
    if (customEmojiPlistName&&customEmojiPlistName.length>0) {
        self.customEmojiDictionary = [[ZCMLEmojiLabelRegexPlistManager sharedInstance]emojiDictForKey:customEmojiPlistName];
    }else{
...
{
    //如果delegate实现了ZCMLEmojiLabel自身的选择link方法
    if(self.delegate&&[self.delegate respondsToSelector:@selector(ZCMLEmojiLabel:didSelectLink:withType:)]){
        if (self.activeLink&&self.activeLink.result.resultType==NSTextCheckingTypeCorrection) {
            ...
            for (NSUInteger i=0; i<kURLActionCount; i++) {
                if ([result.replacementString hasPrefix:zckURLActions[i]]) {
                    NSString *content = [result.replacementString substringFromIndex:zckURLActions[i].length];
                    //type的数组和i刚好对应
                    [self.delegate ZCMLEmojiLabel:self didSelectLink:content withType:i];
...
//    NSString *filePath =  [[NSBundle mainBundle] pathForResource:@"Expression.bundle/expression.json" ofType:nil];
//    if(filePath==nil){
//        return ;
//    }
//    //根据文件路径读取数据
