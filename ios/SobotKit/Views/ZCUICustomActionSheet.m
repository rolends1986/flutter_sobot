...
-(void)setupType:(SatisfactionType)type{
    ...
    if (_isBcakClose && (currentServerType == 4 || currentServerType == 3 || currentServerType == 5)) {// 人工客服返回评价后结束会话
        ...
        if (currentServerType == RobotSatisfcationBackType || currentServerType == ServerSatisfcationBackType) {
            ...
            titlelab.text = ZCSTLocalString(@"机器人客服评价");
        }else if(currentServerType == ServerSatisfcationNolType || currentServerType == ServerSatisfcationOrderType){
            titlelab.text = ZCSTLocalString(@"请您对本次服务进行评价");
        }
...
#pragma mark -- 2.7.4版本新增 留言详情页评价 先计算是否添加 评价输入框，不在时时点击添加
    if (type == 6 && self.textFlag) {
        // 评价输入框
        _textView=[[ZCUIPlaceHolderTextView alloc] initWithFrame:CGRectMake(30, CGRectGetMaxY(self.messageLabel.frame)+20, viewWidth - 60 , 60)];
        ...
        _textView.placeholder         = ZCSTLocalString(@"欢迎给我们的服务提建议~ (选填)");
...
#pragma mark -- 计算首次 显示的内容大小 计算提交按钮的位置
    ...
    CGFloat commitBtY = type>2 ? ((type ==6 && self.textFlag)?CGRectGetMaxY(self.textView.frame)+20 :CGRectGetMaxY(self.messageLabel.frame)+20) : CGRectGetMaxY(self.problemView.frame)+20;
    //三目运算判断
...
-(void)showMenuItem:(BOOL) isShow{
    if (isShow) {
        ...
        if (bgF.size.height > ZCNumber(480-49*4)) {
            bgF.size.height = ZCNumber(480-49*4);
        }
        ...
        [UIView animateWithDuration:0.3 animations:^{
            [self sheetViewSetFrameWithNewF:newSheetViewF];//self.sheetView.frame = newSheetViewF;
        }];
...
-(void)keyBoardWillShow:(NSNotification *) notification{
    ...
    [UIView setAnimationDelegate:self];
    {
        ...
        sheetViewFrame.origin.y = viewHeight - keyboardHeight - self.sheetView.frame.size.height + h;        [self sheetViewSetFrameWithNewF:sheetViewFrame];//self.sheetView.frame = sheetViewFrame;
    }
...
#pragma mark --- 工单留言页面的触发的评价
    if (currentServerType == 6) {
        ...
        __weak ZCUICustomActionSheet * saveSelf = self;
         btn.enabled = false;
        [[[ZCUIConfigManager getInstance] getZCAPIServer] postAddTicketSatisfactionWith:zcLibConvertToString(self.ticketld) Uid:zcLibConvertToString(_config.uid) CompanyId:zcLibConvertToString(_config.companyID) Score:source Remark:textStr start:^{
...
#pragma mark --- 普通评价提交的逻辑
    ...
    //  此处要做是否评价过人工或者是机器人的区分  [ZCIMChat getZCIMChat].libConfig.isArtificial || [[ZCStoreConfiguration getZCParamter:KEY_ZCISOFFLINE] intValue] == 1
...
//        [self closePage:1];
//    }else if(currentServerType == RobotSatisfcationBackType || currentServerType == ServerSatisfcationBackType){// 返回关闭
//        [self closePage:0];
//    }else if(_isBack){
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            [self closePage];
//        });
//    }else if (_isBcakClose && !_isBack ){
//        if (currentServerType == ServerSatisfcationNolType) {
//            [self closePage:1];
//        }else{
//            [self closePage:3];// 用户主动评价人工，并且 开启评价完人工结束会话，并且不是点击 返回按钮的触发事件时，弹新会话键盘，结束本次会话，不退出聊天页面
//        }
//
//    }else{
//        [self closePage:2];
