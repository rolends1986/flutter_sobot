...
+(NSString *) getCheckConfigMsg{
    ...
    if(initInfo){
        ...
        return [NSString stringWithFormat:@"%@|%@|%@|%@|%@|%@|%d|%@|%@",initInfo.appKey,zcLibConvertToString(initInfo.customerCode),initInfo.skillSetId,initInfo.userId,initInfo.receptionistId,initInfo.robotId,initInfo.serviceMode,initInfo.customInfo,initInfo.customerFields];
    }
    return @"";
}
    ...
    switch (buttonIndex) {
        case 2:
        {
            if ([ZCUITools isHasCaptureDeviceAuthorization]) {
                ...
                tipMsg = [tipMsg stringByAppendingString:ZCSTLocalString(@"请在iPhone的“设置-隐私-相机”选项中，允许访问你的手机相机")];
                UIAlertView *alert=[[UIAlertView alloc] initWithTitle:tipMsg message:@"" delegate:nil cancelButtonTitle:ZCSTLocalString(@"好") otherButtonTitles: nil];
                [alert show];
            }
            break;
        }
...
+(void) operateVideo:(NSURL *)videoURL view:(UIView *)zc_sourceView block:(DidFinishPickingMediaBlock)finshBlock{
    ...
    if(totalSecond > 15){
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[ZCUIToastTools shareToast] showToast:ZCSTLocalString(@"最大支持15秒视频!") duration:1.0 view:zc_sourceView  position:ZCToastPositionCenter];
