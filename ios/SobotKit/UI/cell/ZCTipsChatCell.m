...
-(CGFloat) InitDataToView:(ZCLibMessage *) model time:(NSString *) showTime{
    ...
    if(model){
        ...
        if(
           ...
           [zcLibConvertToString(model.sysTips) hasPrefix:ZCSTLocalString(@"排队中，您在队伍中")]    ||
...
-(void)attributedLabel:(ZCTTTAttributedLabel *)label didSelectLinkWithURL:(NSURL *)url{
    ...
    }else if ([label.text hasSuffix:ZCSTLocalString(@"您的留言状态有更新")] && (url.absoluteString.length ==0 || [ZCSTLocalString(@"您的留言状态有更新") isEqual:url.absoluteString])){
...
- (void)HandleHTMLTagsWith:(ZCLibMessage *) model{
    ...
    [ZCHtmlCore filterHtml:text result:^(NSString * _Nonnull text1, NSMutableArray * _Nonnull arr, NSMutableArray * _Nonnull links) {
        ...
        if (text1 != nil && text1.length > 0) {
             _lblTextMsg.attributedText = [ZCHtmlFilter setHtml:text1 attrs:arr view:_lblTextMsg textColor:[ZCUITools zcgetTipLayerTextColor] textFont:[ZCUITools zcgetListKitDetailFont] linkColor:[ZCUITools zcgetChatLeftLinkColor]];
        }else{
    ...
    if ([text hasSuffix:ZCSTLocalString(@"您的留言状态有更新")]) {
        [_lblTextMsg addLinkToURL:[NSURL URLWithString:ZCSTLocalString(@"您的留言状态有更新")] withRange:NSMakeRange(0, ZCSTLocalString(@"您的留言状态有更新").length)];
...
+(NSString *)getSysTipsText:(ZCLibMessage *) model{
    ...
    if ([text hasSuffix:ZCSTLocalString(@"留言")]) {
        // 留言标签的处理
