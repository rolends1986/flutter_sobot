...
@property (nonatomic,assign) int moreCount;// 查看更多
...
+(CGFloat)getCellHeight:(ZCLibMessage *)model time:(NSString *)showTime viewWith:(CGFloat)width{
    ...
    titleLab.textColor = UIColorFromRGB(robotListTextColor);
...
-(CGRect )createTypeItemView:(NSString *) text frame:(CGRect) itemF tag:(int) tag{
    ...
    btn.layer.borderColor = UIColorFromRGB(0xD6DBE5).CGColor;
    btn.titleLabel.lineBreakMode = NSLineBreakByTruncatingTail;
    [btn setTitleColor:UIColorFromRGB(0x3D4966) forState:UIControlStateNormal];
    [btn setBackgroundImage:[self createImageWithColor:UIColorFromRGB(0xEFF4F5)]forState:UIControlStateHighlighted];
