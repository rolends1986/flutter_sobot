...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ZCHotGuideModel *pm = self.tempModel.richModel.hotGuideArr[indexPath.row];
    
    if (_isHistoryMsg) {
        return;
    }
    NSString * question = zcLibConvertToString(pm.question);
    // 发送点击消息
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ZCHotGuideModel  *model = _listArray[indexPath.row];
...
//    thumbnail   title
    ...
    [vcell.labTitle setFont:ListDetailFont];
    [vcell.labTitle setTextColor:UIColorFromRGB(robotListTextColor)];
