...
- (ZCMLEmojiLabel *)lookMoreLabel // 查看更多
{
    if (!_lookMoreLabel) {
...
//        }
    ...
    if (![@"" isEqualToString:zcLibConvertToString(detailDict[@"anchor"])] ||
        ...
        if (self.isRight) {
            ...
            [self.lookMoreLabel setLinkColor:[ZCUITools zcgetChatMultLinkColor]];//UIColorFromRGB(0x4D9DFE);
...
//        dict = nil;
    ...
    if(![@"" isEqualToString:zcLibConvertToString(detailDict[@"anchor"])] ||
       ([@"" isEqualToString: zcLibConvertToString(detailDict[@"thumbnail"])] &&
        ...
        CGSize size = [tempLabel preferredSizeWithMaxWidth:ScreenWidth -160]; // 查看更多的高度
